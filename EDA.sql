...
PORTANTO, TIVEMOSS 14.151 COMPRAS FRAUDULENTAS PARA 136.961 COMPRAS NÃO FRAUDULENTAS*//
...
//* VeriFicação se o mesmo user_id , device_id se repete, ou seja, para ver anormalidades!
 Nessa primeira query, podemos verificar que houveram quantidades de compras maiores de um mesmo 
dispositivo, quando as compras são fraudulentas, se comparadas com a segunda query, quando a 
class=0, ou seja, não fraudulentas. 

primeira query - compras fraudulentas class=1*//
...
//*Segunda query - compras não fraudulentas class=0*//
SELECT class, device_id, COUNT(*) AS Contagem
FROM dbo.Fraud_Data
GROUP BY class, device_id
...
//* Na primeira query, checamos class =1 e verificamos que existem um comportamento de fraude, 
que é fraudar em 1s a partir da criação do cadastro, possivelmente, um boot ou algo do tipo
e já quando a class=0 o comportamento de compra passa a ser no mínimo 137s apos a criação do cadastro*//
